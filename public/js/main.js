...
$(document).ready(function () {
  ...
  $(addButton).click(function () {
    let material = JSON.parse($(this).attr("data"));
    ...
    let fieldHTML =
      ...
      material.map(
        (e) =>
          `<option value=${e.ingredientcode}>${e.name} - ${e.ingredientcode}</option>`
      ) +
      "</select>" +
      '<input required class="border border-primary mb-1 mr-2 " type="number" style="width: 35%" name="materialmass[]" placeholder="mass">' +
      '<i class="fa-solid fa-x delete__item--adddish" style="font-size:15px"></i>' +
      "</div>";
    if (x < maxField) {
...
//remove field input ingredient

$(document).on("click", ".delete__item--adddish", function (e) {
  $(this).parent().remove();
});
...
$(document).on("click", ".btn__delete--imagedish", function (e) {
  imageDelete = [...imageDelete, $(this).attr("id")];
  $(this).parent().remove();
});
